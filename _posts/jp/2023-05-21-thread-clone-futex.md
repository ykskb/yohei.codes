...
これは、スレッド（もしくはpthread）に関連するトピックについて、自分が過去に断続的に取ったメモをまとめたページです。理解を整理するためにまとめました。
...
複数のスレッドがある場合、競合を処理する必要あり。pthreadは`ミューテックス`、`条件変数`、`セマフォ`などの同期プリミティブを提供します。
...
* __Mutex__ (`pthread_mutex_t`): 一度に1つのスレッドのみが特定のコード（メモリ）にアクセスできるようにする。
    ...
    * 実装: 内部状態（カーネルまたはOSがスレッドのblockとwakeupのメカニズムを提供）
